...
class Own_Env_v0(gym.Env):
    ...
    def __init__(self, env_config):
        ...
        # necessary by gym ##
        # oberservation space is a dict, multi binary whether bar exists or not, and box with stiffness goal
        self.observation_space = gym.spaces.Dict(
            {
            'bars': gym.spaces.MultiBinary(parameters.number_bars),
            'stiffness_goal': gym.spaces.Box(high=1.01, low=-1, shape=(9,), dtype=np.float32),
            }
            )
    ...
    def reset(self):
        ...
        try:
            self.stiffness_goal = get_stiffness_goal.stiffness_goal_random_feasible()
            if parameters.check:
    ...
    def step(self, action):
        ...
        if self.done:
            ...
            """define final state"""
            if compute_loss(self.stiffness_fem,
                ...
                try:
                    ...
                    self.info["MSE_loss"] = compute_loss(self.stiffness_fem, self.stiffness_goal).item()
